# ZFI_R_EXTORNOS — Gestión de extornos bancarios (CDI_11)

**Estado: copia base sin adaptar.** Es una copia literal de `ZFI_R_DEVOLUCIONES`
(solo renombrado: report, includes y clase `lcl_devoluciones` → `lcl_extornos`),
punto de partida para implementar RU_02/RU_03 del DF *"Procedimiento Gestión de
extornos"* (CDI_11), siguiendo la sugerencia de EVA en RU_03 de reutilizar el
motor de `ZFI_R_DEVOLUCIONES` (crea + cierra + contabiliza vía `RFKKA00`) en vez
de crear un servicio RFC nuevo.
...
```
src/
  ZFI_R_EXTORNOS.abap        Programa principal (REPORT)
  ZFI_R_EXTORNOS_TOP.abap    Include TOP (tipos y datos globales)
  ZFI_R_EXTORNOS_EVE.abap    Include EVE (pantalla de selección)
  ZFI_R_EXTORNOS_CLS.abap    Include CLS (clase lcl_extornos)
docs/
...
1. Crear el programa **`ZFI_R_EXTORNOS`** (tipo *Report ejecutable*), atributos
   equivalentes a `ZFI_R_DEVOLUCIONES`.
2. Crear los includes **`ZFI_R_EXTORNOS_TOP`**, **`ZFI_R_EXTORNOS_EVE`**,
   **`ZFI_R_EXTORNOS_CLS`** con el contenido de `src/`, e incluirlos en el
...
- Confirmar el nombre definitivo del programa/objetos (`ZFI_R_EXTORNOS` es
  provisional).
